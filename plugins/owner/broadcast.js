   ...
   async: async (m, {
      ...
      try {
         ...
         if (text) {
            ...
            client.reply(m.chat, Func.texted('bold', `🚩 Sukses mengirim broadcast ke ${id.length} ${command == 'bc' ? 'chats' : 'groups'}`), m)
         } else if (/video|image\/(jpe?g|png)/.test(mime)) {
            for (let jid of id) {
               await Func.delay(1500)
               let media = await q.download()
               await client.sendFile(jid, media, '', q.text ? '乂  *Tensura Community*\n\n' + q.text : '', null, null,
                  command == 'bcgc' ? {
                     contextInfo: {
            ...
            client.reply(m.chat, Func.texted('bold', `🚩 Sukses menggirim broadcast ke ${id.length} ${command == 'bc' ? 'chats' : 'groups'}`), m)
         } else client.reply(m.chat, Func.texted('bold', `🚩 Media / text tidak ditemukan.`), m)
      } catch (e) {
