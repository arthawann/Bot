exports.run = {
   ...
   async: async (m, {
      ...
      if (!args || !args[0]) return client.reply(m.chat, `🚩 *Status* : [ ${setting[type] ? 'ON' : 'OFF'} ]\n\nGunakan *on* atau *off* untuk menggunakan fitur ini`, m)
      let option = args[0].toLowerCase()
      let optionList = ['on', 'off']
      if (!optionList.includes(option)) return client.reply(m.chat, `🚩 *Status* : [ ${setting[type] ? 'ON' : 'OFF'} ]\n\nGunakan *on* atau *off* untuk menggunakan fitur ini`, m)
