      ...
      try {
         ...
         if (text) {
            ...
            if (text.match('getstickerpack.com')) {
               ...
               await client.reply(m.chat, Func.texted('bold', `✅ Sukses.`), m)
            } else {
               let rows = []
               ...
               client.sendList(m.chat, '', `Menunjukkan hasil untuk : “${text}”, pilik stiker yang ingin di konvert. 🍟`, '', 'Tap!', [{
                  rows
               }], m)
            }
         } else {
            if (m.quoted ? m.quoted.message : m.msg.viewOnce) {
               ...
               if (/video/.test(type)) {
                  if (q.seconds > 10) return client.reply(m.chat, Func.texted('bold', `🚩 Maksimal durasi video 10 detik.`), m)
               ...
               if (/image\/(jpe?g|png)/.test(mime)) {
                  ...
                  if ((q.msg || q).seconds > 10) return client.reply(m.chat, Func.texted('bold', `🚩 Maksimal durasi video 10 detik.`), m)
                  let img = await q.download()
                  if (!img) return client.reply(m.chat, global.status.wrong, m)
                  return await client.sendSticker(m.chat, img, m, {
                     packname: exif.sk_pack,
