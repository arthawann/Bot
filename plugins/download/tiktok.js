exports.run = {
   ...
   async: async (m, {
      ...
      try {
         if (!args || !args[0]) return client.reply(m.chat, Func.example(isPrefix, command, 'https://www.tiktok.com/t/ZSLNrqUc4/?t=1'), m)
         if (!args[0].match('tiktok.com')) return client.reply(m.chat, global.status.invalid, m)
         client.sendReact(m.chat, '🕒', m.key)
         let old = new Date()
         let json = await Api.tiktok(Func.ttFixed(args[0]))
         if (!json.status || !json.data.video) return client.reply(m.chat, Func.texted('bold', `🚩 Error! private videos or videos not avilable.`), m)
