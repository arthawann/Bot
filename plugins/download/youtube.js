   ...
   async: async (m, {
      ...
      try {
         if (/yt?(a|mp3)/i.test(command)) {
            ...
            const json = await Api.youtube(args[0], 'audio', '128kbps')
            ...
            if (chSize.oversize) return client.reply(m.chat, `💀 File sizenya (${json.data.size}) mau ngebunuh gw? download disini aja : ${await (await scrap.shorten(json.data.url)).data.url}`, m)
            ...
            if (!/^(?:https?:\/\/)?(?:www\.|m\.|music\.)?youtu\.?be(?:\.com)?\/?.*(?:watch|embed)?(?:.*v=|v\/|\/)([\w\-_]+)\&?/.test(args[0])) return client.reply(m.chat, global.status.invalid, m)
            client.sendReact(m.chat, '🕒', m.key)
            var json = await Api.youtube(args[0])
            if (!json.status) {
               var json = await Api.youtube(args[0], 'video', '360p')
            }
            if (!json.status || !json.data.url) return client.reply(m.chat, Func.jsonFormat(json), m)
            ...
            caption += global.footer
            let chSize = Func.sizeLimit(json.data.size, global.max_upload)
            if (chSize.oversize) return client.reply(m.chat, `💀 File sizenya (${json.data.size}) anjir mau ngebunuh gw? download pake ini aja : ${await (await scrap.shorten(json.data.url)).data.url}`, m)
